...
- **Catalogue sync fetches with the `curl` BINARY, not httpx — and the nightly job's clock lives in Python (2026-07-29).** Shopify fronts `madhats.com.au` with Cloudflare, which scores the caller's **TLS fingerprint together with its ASN**. From a hosting ASN the Python TLS stack is refused outright. Measured on the prod droplet (DigitalOcean SYD1), same host, same egress IP, same URL, seconds apart: `httpx` → **429**, `curl_cffi(impersonate="chrome")` → **429**, `curl` binary → **200**. The identical `httpx` call returns **200** from a residential ASN (Superloop), which is the whole trap: **this fails only in production and passes in dev**, and reads like "the sync is broken" or "Shopify rate-limited us". Neither is true — nothing retries its way out of it, and the User-Agent is irrelevant (proven: browser UA 429s too). So `catalogue_sync._curl_get_json` shells out via `asyncio.create_subprocess_exec`, with `-w "\n%{http_code}"` appending the status after the body's last newline; `backend/Dockerfile` installs `curl` and **that line is load-bearing, not a debugging convenience** — remove it and the nightly sync dies on the server while every test still passes. Pagination, `_to_row`, `_map_views` and the delete/insert are unchanged. Failures raise a typed `CatalogueFetchError`.
  **Nightly refresh:** `POST /admin/catalogue/sync-all` (every `active` store with a `shopify_domain`; a broken feed records `ok:false` for that store and the others still sync — one bad feed must not drop nine catalogues) driven by a **`catalogue-sync` compose sidecar** in `docker-compose.prod.yml`, shaped like the existing `watchdog`. **The sidecar does NOT compute midnight** — `curlimages/curl` is Alpine and ships **no tzdata**, so `TZ=Australia/Sydney date` prints **UTC** there and shell-side arithmetic would fire at **10:00 Sydney**, silently, looking correct in the logs (verified). Instead it sleeps on `GET /admin/catalogue/seconds-until-next-sync`, which returns a **bare integer as `text/plain`** (that image has no `jq`, so `secs=$(curl …)` must be a number); a `case "$secs" in ''|*[!0-9]*)` guard falls back to 900s on any garbage/unreachable backend. **`seconds_until_next_sync` must subtract in UTC**: Python ignores the offset when both operands share a `tzinfo` object, so `next_midnight - now` is wall-clock arithmetic and answers 24h for the 23h that actually elapse on a DST-transition night (the job would run at 01:00 once a year) — pinned by a test on the 2026-10-04 AEDT boundary. `ZoneInfo` is resolved **lazily** (`sync_timezone()`), never at import, because Windows ships no tz database and a module-scope `ZoneInfo(...)` took down every import of the module — including an unrelated existing test file; `tzdata` is now a declared dependency. The **manual sync button is untouched** (`POST /admin/stores/{store_id}/sync`, byte-identical).
