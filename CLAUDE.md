...
- **Graphics library (Phase 2 — clipart + company graphics):** store-scoped `graphics` table (`category` `clipart|company`, migration `20260713000003_graphics.sql`), raster images in the private bucket served via the **`/media` proxy** (taint-safe on the canvas). Customer: `GET /graphics?category=` → the tool rail's **"Graphics"** button opens a tabbed (Clipart/Company) **`GraphicsPicker`** modal; clicking a graphic drops it on the canvas as an aspect-preserved image element. Admin: `services/graphics.py`, `POST/GET/DELETE /admin/graphics` (`X-Admin-Secret` + `X-Store-Key`, logo-upload validation) + the admin **Graphics** view (`admin/views/GraphicsView.tsx`, store selector + category tabs + upload + inline-confirm delete). Seed: `python -m app.scripts.seed_clipart` (12 self-authored PNG icons — run inside the backend container; idempotent). Verified live end-to-end: seeded clipart in the picker + admin-uploaded company graphic surfaces in the customer Company tab.
- Tests: backend `pytest` 411, frontend `vitest run` 181 passing (2 pre-existing `adminQuotes` failures, unrelated — missing Router context; on Windows an intermittent tinypool "Worker exited" flake can appear in the full run — rerun focused).
