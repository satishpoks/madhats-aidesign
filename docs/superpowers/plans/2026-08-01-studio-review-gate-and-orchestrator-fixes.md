...
- Baselines to hold (re-measure by stashing before starting; do not trust these numbers): backend flag-off **1259**, v2 suites flag-on **330**, frontend `src/__tests__` + `src/components/StoreHeader.test.tsx` **330**, `docker compose exec -T frontend npx tsc --noEmit` **clean**.
